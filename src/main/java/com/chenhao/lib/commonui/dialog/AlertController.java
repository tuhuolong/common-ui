...
public class AlertController {
    ...
    public AlertController(Context context, DialogInterface di, Window window, int gravity) {
        ...
        mListLayoutWithTitle = R.layout.select_dialog_center;
    ...
    public void setMessage(SpannableStringBuilder builder) {
        mMessageBuilder = builder;
        if (mMessageView != null) {
            mMessageView.setText(mMessageBuilder);
            mMessageView.setHighlightColor(Color.TRANSPARENT);
    ...
    /**
     * Sets a click listener or a message to be sent when the button is clicked. You only need to
     * pass one of {@code listener} or {@code msg}.
     *
    ...
    private void setupView() {
        ...
        if (TextUtils.isEmpty(mTitle) && TextUtils.isEmpty(mMessage) && mMessageBuilder == null) {
    ...
    private boolean setupTitle(LinearLayout topPanel) {
        ...
        if (mCustomTitleView != null) {
            ...
            if (hasTextTitle) {
                ...
                /*
                 * Do this last so that if the user has supplied any icons we use them instead of
                 * the default ones. If the user has specified 0 then make it disappear.
                 */
                if (mIconId > 0) {
                    mIconView.setImageResource(mIconId);
                } else if (mIcon != null) {
    ...
    private void centerButton(TextView button) {
        ...
        button.setBackgroundResource(R.drawable.alert_dialog_button);
    }
    ...
    private void setBackground(LinearLayout topPanel, LinearLayout contentPanel,
        ...
        if (mTransplantBg) {
            ...
            /*
             * We now set the background of all of the sections of the alert. First collect together
             * each section that is being displayed along with whether it is on a light or dark
             * background, then run through them setting their backgrounds. This is complicated
             * because we need to correctly use the full, top, middle, and bottom graphics depending
            ...
            /*
             * The contentPanel displays either a custom text message or a ListView. If it's text we
             * should use the dark background for ListView we should use the light background. If
             * neither are there the contentPanel will be hidden so set it as null.
             */
            ...
            if (lastView != null) {
                if (setView) {
                    ...
                    /*
                     * ListViews will use the Bright background but buttons use the Medium
                     * background.
                     */
    ...
    public static class AlertParams {
        ...
        public void apply(AlertController dialog) {
            ...
            if (mMesageBuilder != null) {
                dialog.setMessage(mMesageBuilder);
            }
            ...
            // adapter or a alert_dialog_cursor
        ...
        private void createCenterListView(final AlertController dialog) {
            final LinearLayout customView = (LinearLayout) mInflater
                    .inflate(dialog.mListLayoutWithTitle, null);
            final RecycleListView listView = (RecycleListView) customView
                    .findViewById(R.id.select_dialog_listview);
            ListAdapter adapter;
            int layout = R.layout.alert_dialog_center_item;
            if (mCursor == null) {
            ...
            /*
             * Don't directly set the adapter on the ListView as we might want to add a footer to
             * the ListView later.
             */
